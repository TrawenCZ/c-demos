...
bool eliminate_row(unsigned int sudoku[9][9], int row_index) {
  ...
  for (int col = 0; col < 9; col++) {
    if (!bitset_is_unique(sudoku[row_index][col]))
      continue;
  ...
  for (int col = 0; col < 9; col++) {
    ...
    if (sudoku[row_index][col] != holder) {
      sth_changed = true;
    }
  }
  return sth_changed;
...
bool eliminate_col(unsigned int sudoku[9][9], int col_index) {
  ...
  for (int row = 0; row < 9; row++) {
    if (!bitset_is_unique(sudoku[row][col_index]))
      continue;
  ...
  for (int row = 0; row < 9; row++) {
    ...
    if (sudoku[row][col_index] != holder) {
      sth_changed = true;
    }
  }
...
static bool valid_box_bitset(unsigned int sudoku[9][9], int row_index,
                             int col_index) {
  unsigned int bitset = 0;
  for (int row = row_index; row < row_index + 3; row++) {
    for (int col = col_index; col < col_index + 3; col++) {
      if (!bitset_is_unique(sudoku[row][col]))
        continue;
      if (bitset_is_set(bitset, sudoku[row][col])) {
        return false;
      }
      bitset += sudoku[row][col];
    }
...
bool is_valid(unsigned int sudoku[9][9]) {
  ...
  for (int row = 0; row < 7; row += 3) {
    for (int col = 0; col < 7; col += 3) {
      if (!valid_box_bitset(sudoku, row, col)) {
        return false;
      }
    }
  }
  ...
  for (int row = 0; row < 9; row++) {
    ...
    for (int col = 0; col < 9; col++) {
      ...
      if (bitset_is_set(row_bitset, sudoku[row][col]) ||
          bitset_is_set(cols_bitsets[col], sudoku[row][col])) {
        return false;
      }
...
bool solve(unsigned int sudoku[9][9]) {
  ...
  bool row_changes = false, col_changes = false, box_changes = false,
       entering_while = true;

  while (row_changes || col_changes || box_changes || entering_while) {
    row_changes = false, col_changes = false, box_changes = false,
    entering_while = false;
    for (int row = 0; row < 9; row++) {
      row_changes = eliminate_row(sudoku, row) || row_changes;
      if (!is_valid(sudoku))
        return false;
    }
    for (int col = 0; col < 9; col++) {
      col_changes = eliminate_col(sudoku, col) || col_changes;
      if (!is_valid(sudoku))
        return false;
    }
    for (int row = 0; row < 7; row += 3) {
...
bool load_error(int input) {
  ...
  case 3:
    fprintf(stderr,
            "Positon of 'New Line' separator taken by another symbol.\n");
    break;
  case 4:
    ...
    default:
      ...
      while (abs_col < 26) {
        ...
        default:
          ...
          if (loaded >= '1' && loaded <= '9') {
            sudoku[row][col] = (1 << (loaded - '1'));
            valid = true;
          }
          if (!valid)
            return load_error(5);
...
bool load(unsigned int sudoku[9][9]) {
  ...
  while ((loaded = getchar()) != EOF) {
    if (loaded == '+') {
      return load_long(sudoku);
    } else if (loaded >= '0' && loaded <= '9') {
      return load_short(sudoku, loaded);
    } else if (loaded != '\n')
      return load_error(9);
  }
  return load_error(9);
...
void generate(unsigned int sudoku[9][9]) {
  if (!is_valid(sudoku))
    return;
  unsigned int *raw = (unsigned int *)sudoku;
  unsigned int sudoku_copy[9][9];
  bool finished;
  unsigned int value_holder;
  int used_indexes[81] = {0};

  for (int i = 0; i < 81; i++) {
    if (!bitset_is_unique(raw[i])) {
      used_indexes[i] = 1;
    }
  }
  ...
  while (true) {
    ...
    int ran_index = rand() % 81; // random index between 0 and 80 (81 is length
                                 // of raw format for sudoku)
    if (used_indexes[ran_index] == 1)
      continue;

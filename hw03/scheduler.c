...
bool copy_queue(priority_queue *dest, const priority_queue *source) 
{
    ...
    while (pSourceNode != NULL) {
        ...
        if (dest->top == NULL) {dest->top = pNewDestNode;}
        else {pDestNode->next = pNewDestNode; }     // pDestNode is not NULL
        memcpy(&(pNewDestNode->process), &(pSourceNode->process), sizeof(process_type));
        pNewDestNode->prev = pDestNode;
        pDestNode = pNewDestNode; pSourceNode = pSourceNode->next;
    }
    if (pDestNode != NULL) pDestNode->next = NULL;
...
void clear_queue(priority_queue *queue) 
{
    ...
    queue->top = NULL; queue->bottom = NULL; queue->size = 0;
}
...
int bit_count(uint16_t cpu_mask) {
    ...
    while (cpu_mask >= bit_pointer) {
        if ((bit_pointer & cpu_mask) != 0) count += 1;
        bit_pointer <<= 1;
    }
    return count;
}

enum push_result pusher(priority_queue *queue, priority_queue_item *pPushPosition, process_type process, bool atEnd) 
{
    priority_queue_item *pushNode = malloc(sizeof(priority_queue_item));
    if (pushNode == NULL) return push_error;

    if (pPushPosition == NULL) {
        pushNode->next = NULL; 
        if (atEnd) {queue->bottom->next = pushNode; pushNode->prev = queue->bottom; queue->bottom = pushNode; }
        else {pushNode->prev = NULL; queue->top = pushNode; }
    } else if (queue->top == pPushPosition) {pushNode->next = pPushPosition; queue->top = pushNode; pPushPosition->prev = pushNode;}
    else {pushNode->next = pPushPosition; pushNode->prev = pPushPosition->prev; pPushPosition->prev->next = pushNode; pPushPosition->prev = pushNode; }

    if (queue->bottom == NULL) queue->bottom = pushNode;
    memcpy(&(pushNode->process), &process, sizeof(process));
    return push_success;
}
...
enum push_result push_to_queue(priority_queue *queue, process_type process)
{
    ...
    while (pNode != NULL) {
        if (!positionFound && pNode->process.niceness * pNode->process.remaining_time >= process.niceness * process.remaining_time) {
            pPushPosition = pNode;
            positionFound = true;
        }
        empty = false;
        if (!(pNode->process.callback == process.callback && pNode->process.context == process.context)) {pNode = pNode->next; continue;}

        if (pNode->process.niceness * pNode->process.remaining_time == process.niceness * process.remaining_time && 
            pNode->process.cpu_mask == process.cpu_mask) return push_duplicate;
        return push_inconsistent;
    }

    unsigned int inputPriority = process.niceness * process.remaining_time;
    if (pPushPosition == NULL) {
        if (!empty) return pusher(queue, pPushPosition, process, true);
        return pusher(queue, pPushPosition, process, false);
    }
    
    if (pPushPosition->process.niceness * pPushPosition->process.remaining_time > inputPriority) {
        return pusher(queue, pPushPosition, process, false);
    } else {
        unsigned int sourcePriority = pPushPosition->process.niceness * pPushPosition->process.remaining_time;
        while (inputPriority == sourcePriority && bit_count(pPushPosition->process.cpu_mask) < bit_count(process.cpu_mask)) {
            pPushPosition = pPushPosition->next;
            sourcePriority = pPushPosition->process.niceness * pPushPosition->process.remaining_time;
        }
